 ...
 *
 *  五路非阻塞任务:
 *    10ms  → MPU6050 读取 + 互补滤波 + 喂狗
 *    20ms  → PID 计算 + 舵机更新
 *    50ms  → VOFA+ 数据帧 (7通道) + DMA 状态
 *   200ms  → ADC 触发 + 编码器读取 + 按键检测 + 模式切换
 *  1000ms  → OLED 显示刷新
 */
...
/* ---------- 设定值 ---------- */
static float      setpoint_angle = 0.0f;   /* 目标俯仰角, 初始 0° (水平) */
static int16_t    encoder_delta;
static int32_t    encoder_accum;

/* ---------- 调参模式 ---------- */
/*
 * 短按编码器 SW 按键 (PB14) 切换模式:
 *   MODE_RUN     (0) — 正常运行, 编码器调 setpoint
 *   MODE_TUNE_KP (1) — 编码器调 Kp
 *   MODE_TUNE_KI (2) — 编码器调 Ki
 *   MODE_TUNE_KD (3) — 编码器调 Kd
 * 在调参模式下, OLED 底部显示当前参数名和值.
 */
#define MODE_RUN      0
...
/* ============================================================
 *              初 始 化 IWDG 独 立 看 门 狗
 * ============================================================
 */
/**
 * LSI 约 40kHz, 预分频 64 → 625Hz
 * 重装载值 625 → 超时约 1 秒
 * 在 10ms 任务中喂狗 → 100Hz, 远快于 1s 超时, 安全裕度大
 */
...
int main(void)
{
    ...
    /* ADC 的电压值 (mV), 在 200ms 任务中更新, OLED 和 VOFA+ 使用 */
    static uint16_t adc_mv = 0;
    ...
    while (1)
    {
        ...
        if (now - last_50ms >= 50)
        {
            ...
            vofa_data[6] = (float)adc_mv / 1000.0f;   /* ADC 电压 (V) */

            VOFA_SendFrame(vofa_data, 7);
        }

        /* ========================================
         *  Task_200ms (5Hz) — ADC + 编码器 + 模式切换
         * ======================================== */
